...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # when all corners are visited we will reached the Goal state
        if len(state[1]) == 0:
            return True
        else:
            return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if hitsWall is False:
                new_state = (nextx, nexty)  # getting to the next state
                cost = 1
                # check if the next state is in one of the corner positions or not
                if new_state in self.corners:
                    if new_state in visitedCorner:
                        remain = list(visitedCorner)
                        remain.remove(new_state)  # updating nodes remaining to visit
                        visitedCorner = tuple(remain)

                successors.append(((new_state, visitedCorner), action, cost))
...
def foodHeuristic(state, problem):
    ...
    for food in uneatenList:
        ...
        heuristic_val.append(mazeDistance(position, food, problem.startingGameState))
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        curr = self.food.asList()
        # if there exists food at the current location return true
        if (x, y) in curr:
            return True
        else:
            return False
